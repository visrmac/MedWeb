...
from appointment.models import Invoice
        ...
        if form.is_valid():
            ...
            try:
                ...
                try:
                    ...
                    url = ""
                new = UserProfile(phone=request.POST['phone'], name=request.POST['name'], email=request.POST['email'],
                          age=request.POST['age'], address=request.POST['address'],
                          gender=request.POST['gender'], blood_group=request.POST['blood_group'], case_paper=request.POST['case_paper'],
                          medical=url)
                new.save()
...
def UpdatedUserProfile(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            pass
        form.save()
        extra_update = Invoice.objects.get(patient=request.user)
